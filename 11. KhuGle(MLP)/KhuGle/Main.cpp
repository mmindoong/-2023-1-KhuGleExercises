...
class CDnnTest : public CKhuGleWin
{
public:
	CKhuGleGraphLayer *m_pTrainGraphLayer, *m_pTestGraphLayer;
	CKhuDaNet m_DnnNetwork;
	bool m_bTrainingRun;
	...
	CDnnTest(int nW, int nH, char *ExePath);
	~CDnnTest();
	void LoadMnistTrain();
	void LoadMnistTest();
	void Update();
...
CDnnTest::CDnnTest(int nW, int nH, char *ExePath) : CKhuGleWin(nW, nH) 
{
	...
	m_DnnNetwork.AddLayer(CKhuDaNetLayerOption(KDN_LT_INPUT | KDN_LT_FC, 0, 28*28, 0, 0, 0, 0, 0.15));
	m_DnnNetwork.AddLayer(CKhuDaNetLayerOption(KDN_LT_FC, 0, 512, 0, 0, 0, KDN_AF_RELU, 0.15));
	m_DnnNetwork.AddLayer(CKhuDaNetLayerOption(KDN_LT_FC, 0, 100, 0, 0, 0, KDN_AF_RELU, 0.15));
	m_DnnNetwork.AddLayer(CKhuDaNetLayerOption(KDN_LT_FC | KDN_LT_OUTPUT, 0, 10, 0, 0, 0, KDN_AF_SOFTMAX, 0.15));

	m_DnnNetwork.InitWeight();
	...
	std::cout << m_DnnNetwork.GetInformation() << std::endl;
...
void CDnnTest::Update()
{
	...
	{
		m_pScene->Render();
		DrawSceneTextPos("MLP (DNN) Test", CKgPoint(0, 0));
	...
	double Loss;
	int nTP = m_DnnNetwork.TrainBatch(m_MnistTrainInput+nIndex, OutputList, m_nBatch, &Loss);
	...
	if(nIndex+m_nBatch == m_nMnistTrainTotal)
	{
		...
		for(i = 0 ; i < m_nMnistTestTotal ; i++)
		{
			int nResult = m_DnnNetwork.Forward(m_MnistTestInput[i]);
...
int main()
{
	...
	CDnnTest *pDnnTest = new CDnnTest(640, 480, ExePath);

	KhuGleWinInit(pDnnTest);
